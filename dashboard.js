...
async function sbPost(table, data){
  ...
  if(!r.ok){ var err=await r.text(); throw new Error(r.status+': '+err); }
  return r.json();
}
...
window.sauverNotesPrivees = async function(){
  ...
  if(!cl){ toast2('Aucun client'); return; }
  var motiv = (document.getElementById('notes-motivation')||{}).value||'';
  var vic   = (document.getElementById('victoires-input')||{}).value||'';
  var vig   = (document.getElementById('vigilance-input')||{}).value||'';
  var priv  = (document.getElementById('notes-privees')||{}).value||'';
  try{
    await sbUpsert('ecran_client',{
      client_id: cl.id,
      note_motivation: motiv, note_visible: motiv,
      victoires: JSON.stringify(vic.split('\n').map(function(s){return s.trim();}).filter(Boolean)),
      points_attention: JSON.stringify(vig.split('\n').map(function(s){return s.trim();}).filter(Boolean)),
      notes_privees: priv
    },'client_id');
...
window.openProfile = function(id){
  ...
  // Reset les jours programme
  PROG_JOURS_DATA = [{nom:'Jour 1',muscles:'',exos:[]},{nom:'Jour 2',muscles:'',exos:[]},{nom:'Jour 3',muscles:'',exos:[]}];
  PROG_JOUR_ACTIF = 0;
...
function renderAgendaRDV(rdvs){
  ...
  var upcoming = (rdvs||[])
    .filter(function(r){ return r.date_rdv && r.date_rdv.substr(0,10) >= new Date().toISOString().split('T')[0]; })
...
window.analyserClientData = async function(){
  ...
  var nom_mois=['janvier','février','mars','avril','mai','juin','juillet','août','septembre','octobre','novembre','décembre'][new Date().getMonth()];
  try{
    var mois_debut = new Date(new Date().getFullYear(),new Date().getMonth(),1).toISOString().split('T')[0];
    var xp=[]; try{xp=await sbFetch('xp_log?client_id=eq.'+cl.id+'&created_at=gte.'+mois_debut+'&select=source');}catch(e){}
    var nb=xp.filter(function(x){return x.source==='seance_exo';}).length;
    if(nb>=8) sugg_vic.push(nb+' entrainements en '+nom_mois+' — super !');
    else if(nb>=4) sugg_vic.push(nb+' seances en '+nom_mois+', continue !');
    else if(nb>=1) sugg_vic.push(nb+' seance(s) en '+nom_mois+' — bon debut !');
    else { sugg_vic.push('Nouveau mois, nouvelle chance !'); sugg_vig.push('0 seance ce mois — a rattraper !'); }
    sugg_vic.push('A completer : Record [exercice] — [charge]kg');
    sugg_vig.push('A completer : Baisse sur [exercice]');
    var vl=document.getElementById('sugg-vic-list');
    var gl=document.getElementById('sugg-vig-list');
    if(vl){ vl.innerHTML=''; sugg_vic.forEach(function(s){ var b=document.createElement('button'); b.textContent=s; b.style.cssText='padding:5px 10px;background:rgba(76,175,122,0.15);border:1px solid rgba(76,175,122,0.4);border-radius:20px;color:#4CAF7A;font-size:10px;cursor:pointer;font-family:inherit;margin:2px;'; b.onclick=function(){ajouterSuggestion('victoires-input',s);}; vl.appendChild(b); }); }
    if(gl){ gl.innerHTML=''; sugg_vig.forEach(function(s){ var b=document.createElement('button'); b.textContent=s; b.style.cssText='padding:5px 10px;background:rgba(232,149,58,0.15);border:1px solid rgba(232,149,58,0.4);border-radius:20px;color:#E8953A;font-size:10px;cursor:pointer;font-family:inherit;margin:2px;'; b.onclick=function(){ajouterSuggestion('vigilance-input',s);}; gl.appendChild(b); }); }
    toast2('Suggestions generees');
