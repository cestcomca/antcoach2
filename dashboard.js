...
window.enregistrerRdv = async function(){
  try{
    ...
    sbFetch('rdv_coach?date_rdv=gte.'+new Date().toISOString().split('T')[0]+'&select=*,client:profiles!client_id(prenom,nom,email)&order=date_rdv.asc')
      .then(function(rdvs){
...
async function loadAgendaData(){
  ...
  sbFetch('rdv_coach?select=*,client:profiles!client_id(prenom,nom,email)&order=date_rdv.asc&limit=50')
    .then(function(rdvs){
      if(!Array.isArray(rdvs)) return;
      RDV_LIST = rdvs;
      // Toujours mettre à jour la liste RDV
      renderAgendaRDV(rdvs);
    })
