...
window.editExo=function(gi,ei){
  ...
  // Modal photo
  var useNewPhoto=confirm('Changer la photo ? (Annuler = garder l\'actuelle)');
  if(useNewPhoto){
    ...
    inp.onchange=function(){
      ...
      reader.onload=function(ev){
        MUSCU[gi].e[ei]={nom:n.trim(),note:nt.trim(),photo:ev.target.result};
        saveMyExosToLS();
    ...
    MUSCU[gi].e[ei]={nom:n.trim(),note:nt.trim(),photo:photo};
    saveMyExosToLS();
    document.getElementById('pc-inner').innerHTML=dbHTML();
    toast2('Exercice modifié ✓');
  }
