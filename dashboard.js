...
window.assignProg = function(idx){
  ...
  // Rafraîchir la vue
  document.getElementById('pc-inner').innerHTML = programmesHTML();
  // Si la fiche du client assigné est ouverte, recharger ses jours
...
window.editExo=function(gi,ei){
  ...
  // Stocker l'index pour la mise à jour photo
  window._editingExo={gi:gi,ei:ei,nom:n.trim(),note:nt.trim(),photo:photo};
  // Ouvrir modal d'édition photo
  ...
  if(!modal){
    ...
    modal.innerHTML='<div style="background:var(--c1);border-radius:16px;padding:20px;width:90%;max-width:340px;">'
      +'<div style="font-size:13px;font-weight:600;margin-bottom:14px;">Photo de l\u0027exercice</div>'
      +(photo?'<img id="edit-exo-current-photo" src="'+photo+'" style="width:100%;height:120px;object-fit:cover;border-radius:10px;margin-bottom:10px;">':'')
      +'<input type="file" id="edit-exo-photo-input" accept="image/*" style="width:100%;font-size:12px;color:var(--txm);margin-bottom:14px">'
      +'<div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">'
      +'<button onclick="saveEditExo(false)" style="padding:10px;background:var(--c2);border:none;border-radius:10px;color:var(--tx);font-size:12px;font-weight:600;cursor:pointer;font-family:inherit">Garder photo</button>'
      +'<button onclick="saveEditExo(true)" style="padding:10px;background:var(--r);border:none;border-radius:10px;color:#fff;font-size:12px;font-weight:600;cursor:pointer;font-family:inherit">Enregistrer</button>'
      +'</div></div>';
    document.body.appendChild(modal);
  } else {
    // Mettre à jour la photo courante
    var curPhoto=document.getElementById('edit-exo-current-photo');
    if(curPhoto) curPhoto.src=photo;
