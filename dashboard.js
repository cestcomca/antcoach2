...
window.assignProg = function(idx){
  idx = parseInt(idx);
  ...
  saveProgToLS();
  // Sauvegarder en Supabase pour le client
  if(typeof sbUpsert !== 'undefined'){
    progCopy.jours.forEach(function(j){
...
window.openProfile = async function(id){
  ...
  // ── Charger le programme depuis Supabase ──
  try{
    var data = await sbFetch('programmes_clients?client_id=eq.'+id+'&select=*&order=ordre.asc,id.asc');
  ...
  try{
    ...
    if(notes && notes[0]){
      var nm = document.getElementById('notes-motivation'); if(nm) nm.value = notes[0].note_motivation||notes[0].note_visible||'';
      var vi = document.getElementById('victoires-input');  if(vi) vi.value = (JSON.parse(notes[0].victoires||'[]')).join('\n');
      var vg = document.getElementById('vigilance-input');  if(vg) vg.value = (JSON.parse(notes[0].points_attention||'[]')).join('\n');
      var np = document.getElementById('notes-privees');    if(np) np.value = notes[0].notes_privees||'';
    }
  }catch(e){}
};
...
window.analyserClientData = async function(){
  ...
  try{
    ...
    var xp = await sbFetch('xp_log?client_id=eq.'+cl.id+'&created_at=gte.'+mois_debut+'&select=source');
    ...
    else sugg_vig.push('Seulement '+nb_seances+' séance(s) ce mois');
    if(nb_defis>=20) sugg_vic.push(nb_defis+' défis validés ce mois 💪');
    else if(nb_defis<5) sugg_vig.push('Peu de défis validés ('+nb_defis+')');
    var suivi = await sbFetch('suivi_quotidien?client_id=eq.'+cl.id+'&date_suivi=gte.'+mois_debut+'&select=eau_verres,sommeil_h,pas');
    if(suivi&&suivi.length){
      ...
      if(eau>=6) sugg_vic.push('Hydratation top : '+eau.toFixed(1)+' verres/j en moy. 💧');
      else if(eau<4) sugg_vig.push('Hydratation insuffisante : '+eau.toFixed(1)+' verres/j');
      if(som>=7.5) sugg_vic.push('Excellent sommeil : '+som.toFixed(1)+'h/nuit 😴');
      else if(som<6.5) sugg_vig.push('Manque de sommeil : '+som.toFixed(1)+'h/nuit');
      if(pas>=8000) sugg_vic.push('Activité top : '+Math.round(pas).toLocaleString('fr')+' pas/j 👟');
      else if(pas<4000&&pas>0) sugg_vig.push('Peu d\u2019activite : '+Math.round(pas).toLocaleString('fr')+' pas/j');
    }
    var pesees = await sbFetch('pesees?client_id=eq.'+cl.id+'&order=date_pesee.asc&select=poids_kg&limit=20');
    if(pesees&&pesees.length>=2){
      var diff=(pesees[pesees.length-1].poids_kg-pesees[0].poids_kg).toFixed(1);
      if(diff<=-2) sugg_vic.push('Perte de '+Math.abs(diff)+'kg depuis le début 📉');
      if(diff>=2)  sugg_vic.push('Prise de '+diff+'kg depuis le début 💪');
    }
  }catch(e){
    sugg_vic=['8 entraînements ce mois — super !','Hydratation au top 💧'];
    sugg_vig=['Manque d\'assiduité cette semaine','Sommeil en baisse'];
  }
  sugg_vic.push('Record [exercice] : [charge]kg ✓');
  sugg_vig.push('Baisse de charge sur [exercice] : -[X]kg');
  ...
  if(vl){
    ...
    sugg_vic.forEach(function(s){
      ...
      btn.textContent=s;
      btn.style.cssText='padding:5px 10px;background:rgba(76,175,122,0.15);border:1px solid rgba(76,175,122,0.4);border-radius:20px;color:#4CAF7A;font-size:10px;cursor:pointer;font-family:inherit;margin:2px;';
  ...
  if(gl){
    ...
    sugg_vig.forEach(function(s){
      var btn=document.createElement('button');
      btn.textContent=s;
      btn.style.cssText='padding:5px 10px;background:rgba(232,149,58,0.15);border:1px solid rgba(232,149,58,0.4);border-radius:20px;color:#E8953A;font-size:10px;cursor:pointer;font-family:inherit;margin:2px;';
