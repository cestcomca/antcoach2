...
window.assignProg = async function(progIdx){
  ...
  try{
    ...
    // 2. Insérer tous les jours d'un coup
    ...
    var res = await fetch(SB_URL+'/rest/v1/programmes_clients', {
      method: 'POST',
      headers: {
        'apikey': SB_KEY,
    ...
    if(!res.ok){
      ...
      toast2('Erreur Supabase: '+res.status);
      console.error('assignProg error:', err);
      return;
    }
    toast2('Programme "'+prog.nom+'" assigné à '+cl.name+' ✓');
    // 3. Si la fiche client est ouverte sur ce client → recharger
    if(activeClientIdx === ci){
      var panel = document.getElementById('profile-panel');
      if(panel && panel.classList.contains('show')){
        var data = await sbFetch('programmes_clients?client_id=eq.'+cl.id+'&select=*&order=ordre.asc,id.asc');
        if(data && data.length > 0){
          ...
          renderProgDayTabs();
          loadProgJour(0);
          toast2('Fiche client mise à jour ✓');
        }
      }
    }
  }catch(e){
    console.error('assignProg catch:', e);
    toast2('Erreur: '+e.message);
  }
};
