...
window.sauverNotesPrivees = async function(){
  ...
  if(!cl) return;
  var notes    = document.getElementById('notes-privees')?.value || '';
...
window.addExoToProg = function(){
  ...
  var exo = {
    nom:    sel.value,
    series: (series && series.value) ? series.value : '3',
    reps:   (reps   && reps.value)   ? reps.value   : '10',
    repos:  (repos  && repos.value)  ? repos.value  : '90s',
    charge: (charge && charge.value) ? charge.value : '',
  ...
  row.style.cssText = 'background:var(--c2);border-radius:10px;padding:10px 12px;margin-bottom:8px;';
  row.dataset.exo = JSON.stringify(exo);
  row.innerHTML = '<div style="display:flex;align-items:center;justify-content:space-between;">'
    + '<div style="flex:1">'
    + (exo.photo ? "<img src=" + exo.photo + " style=width:60px;height:42px;object-fit:cover;border-radius:6px;margin-bottom:6px;display:block>" : "")
    ...
    + (exo.note ? ' · ' + exo.note : '')
    + '</div></div>'
    + '<button onclick="this.parentNode.parentNode.remove()" style="background:none;border:none;cursor:pointer;color:var(--txm);margin-left:10px"><i class="ti ti-trash" style="font-size:14px"></i></button>'
    + '</div>';
...
window.saveProgramme = async function(){
  ...
  var exos = [];
  if(list){
    list.querySelectorAll('div[data-exo]').forEach(function(row){
      try{ exos.push(JSON.parse(row.dataset.exo)); }catch(e){}
    });
  }
...
window.openProfile = function(id){
  ...
  setTimeout(async function(){
    try{
      var cl = CLIENTS[activeClientIdx];
      if(!cl) return;
      var data = await sbFetch('ecran_client?client_id=eq.'+cl.id+'&select=*&limit=1');
      if(data && data[0]){
...
// ══ AGENDA — CHANGEMENT VUE ══
window.changeVueAgenda = function(vue){
  var inner = document.getElementById('pc-inner');
  if(inner) inner.innerHTML = agendaView(vue);
};
...
function logout(){closeCoachModal();window.location.href='hub.html';}
