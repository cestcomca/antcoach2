...
async function sbDelete(table, filterQuery){
  var session=getSBSession();
  var token=session?session.access_token:SB_KEY;
  var url=SB_URL+'/rest/v1/'+table+'?'+filterQuery;
  const r = await fetch(url, {
    method:'DELETE',
...
async function sbInsertBulk(table, rows){
  var session=getSBSession();
  var token=session?session.access_token:SB_KEY;
  const r = await fetch(SB_URL+'/rest/v1/'+table, {
    method:'POST',
...
window.enregistrerRdv = async function(){
  try{
    ...
    setTimeout(function(){
      ...
      if(inner) inner.innerHTML = agendaView(window._agendaVueActive || 'semaine');
      var d2 = document.getElementById('rdv-date');
      if(d2) d2.value = new Date().toISOString().split('T')[0];
...
window.sauverNotesPrivees = async function(){
  var clientId = CLIENTS[activeClientIdx||0]?.id;
  if(!clientId){ toast2('Notes sauvegardées'); return; }
  var vicRaw = document.getElementById('victoires-input')?.value || '';
  var vigRaw = document.getElementById('vigilance-input')?.value || '';
  var motiv = document.getElementById('notes-motivation')?.value || '';
  var priv = document.getElementById('notes-privees')?.value || '';
  var victoires = vicRaw.split('\n').map(function(l){return l.trim();}).filter(function(l){return l.length>0;});
  var vigilance = vigRaw.split('\n').map(function(l){return l.trim();}).filter(function(l){return l.length>0;});
  try{
    await sbUpsert('ecran_client', {
      client_id: clientId,
      victoires: JSON.stringify(victoires),
      points_attention: JSON.stringify(vigilance),
      note_motivation: motiv,
      notes_privees: priv
    }, 'client_id');
    toast2('Sauvegardé ✓ — visible côté client');
  }catch(e){
    console.error('sauverNotesPrivees:', e);
    toast2('Erreur sauvegarde (voir console)');
  }
};

// ── Analyse automatique (bouton ⚡ Analyser) ──
window.analyserClientData = async function(){
  var clientId = CLIENTS[activeClientIdx||0]?.id;
  if(!clientId){ toast2('Aucun client sélectionné'); return; }
  toast2('Analyse en cours...');
  var vicSugg = [], vigSugg = [];
  try{
    var today = new Date();
    var moisDebut = new Date(today.getFullYear(), today.getMonth(), 1).toISOString().split('T')[0];

    // Séances validées ce mois
    var xplog = await sbFetch('xp_log?client_id=eq.'+clientId+'&created_at=gte.'+moisDebut+'&source=eq.seance_exo&select=id');
    var nbSeances = (xplog||[]).length;
    if(nbSeances >= 12) vicSugg.push(nbSeances+' séances complétées ce mois, régularité exemplaire !');
    else if(nbSeances >= 8) vicSugg.push(nbSeances+' séances ce mois, très bonne régularité');
    else if(nbSeances >= 4) vicSugg.push(nbSeances+' séances ce mois, continue comme ça');
    else if(nbSeances >= 1) vicSugg.push(nbSeances+' séance(s) ce mois-ci');
    else vicSugg.push('Nouveau mois, nouvelle chance de se lancer');

    // Suivi quotidien (eau/sommeil/pas)
    var suivi = await sbFetch('suivi_quotidien?client_id=eq.'+clientId+'&date_suivi=gte.'+moisDebut+'&select=*');
    if(suivi && suivi.length){
      var moyEau = suivi.reduce(function(a,s){return a+(s.eau_verres||0);},0)/suivi.length;
      var moySommeil = suivi.reduce(function(a,s){return a+(parseFloat(s.sommeil_h)||0);},0)/suivi.length;
      if(moyEau >= 7) vicSugg.push('Très bonne hydratation en moyenne ce mois');
      else if(moyEau < 4 && moyEau > 0) vigSugg.push('Hydratation insuffisante, moyenne de '+moyEau.toFixed(1)+' verres/jour');
      if(moySommeil >= 7.5) vicSugg.push('Sommeil de qualité, moyenne de '+moySommeil.toFixed(1)+'h');
      else if(moySommeil < 6 && moySommeil > 0) vigSugg.push('Sommeil insuffisant, moyenne de '+moySommeil.toFixed(1)+'h/nuit');
    }

    // Poids - tendance
    var pesees = await sbFetch('pesees?client_id=eq.'+clientId+'&order=date_pesee.desc&select=poids_kg,date_pesee&limit=6');
    if(pesees && pesees.length >= 2){
      var diff = pesees[0].poids_kg - pesees[pesees.length-1].poids_kg;
      if(Math.abs(diff) >= 0.1){
        var mot = diff < 0 ? 'Perte de poids de '+Math.abs(diff).toFixed(1)+'kg sur les dernières pesées' : 'Prise de poids de '+diff.toFixed(1)+'kg sur les dernières pesées';
        vicSugg.push(mot);
      }
    } else if(!pesees || !pesees.length){
      vigSugg.push('Aucune pesée enregistrée récemment');
    }

    // Rendu tags cliquables
    var vicList = document.getElementById('sugg-vic-list');
    var vigList = document.getElementById('sugg-vig-list');
    if(vicList) vicList.innerHTML = vicSugg.map(function(s){
      return '<span data-sugg-vic="'+encodeURIComponent(s)+'" style="padding:5px 10px;background:rgba(76,175,122,0.12);border:0.5px solid rgba(76,175,122,0.3);border-radius:16px;font-size:11px;color:#4CAF7A;cursor:pointer">'+s+'</span>';
    }).join('');
    if(vigList) vigList.innerHTML = vigSugg.length ? vigSugg.map(function(s){
      return '<span data-sugg-vig="'+encodeURIComponent(s)+'" style="padding:5px 10px;background:rgba(232,149,58,0.12);border:0.5px solid rgba(232,149,58,0.3);border-radius:16px;font-size:11px;color:var(--or);cursor:pointer">'+s+'</span>';
    }).join('') : '<span style="font-size:11px;color:var(--txm)">Rien à signaler</span>';

    // Listeners clic pour ajouter au textarea
    if(vicList) vicList.querySelectorAll('[data-sugg-vic]').forEach(function(el){
      el.addEventListener('click', function(){
        var txt = decodeURIComponent(el.getAttribute('data-sugg-vic'));
        var ta = document.getElementById('victoires-input');
        if(ta) ta.value = (ta.value ? ta.value+'\n' : '') + txt;
        toast2('Ajouté aux victoires');
      });
    });
    if(vigList) vigList.querySelectorAll('[data-sugg-vig]').forEach(function(el){
      el.addEventListener('click', function(){
        var txt = decodeURIComponent(el.getAttribute('data-sugg-vig'));
        var ta = document.getElementById('vigilance-input');
        if(ta) ta.value = (ta.value ? ta.value+'\n' : '') + txt;
        toast2('Ajouté en vigilance');
      });
    });

    toast2('Analyse terminée ✓');
  }catch(e){
    console.error('analyserClientData:', e);
    toast2('Erreur analyse (voir console)');
  }
};
...
window.sauverDiete = async function(){
  ...
  var clientId = CLIENTS[activeClientIdx||0]?.id;
  if(!clientId || !kcal){ toast2('Renseigne au moins les kcal'); return; }
  try{
    // Approche robuste : supprimer l'ancienne diète active puis réinsérer
    await sbDelete('dietes', 'client_id=eq.'+clientId);
    var res = await sbInsertBulk('dietes', [{
      client_id:clientId,
    ...
    if(!res.ok){
      console.error('sauverDiete erreur:', res.status, await res.text());
      toast2('Erreur sauvegarde diète');
      return;
    }
    toast2('Diète enregistrée ✓ — visible côté client');
  }catch(e){
    console.error('sauverDiete:', e);
    toast2('Erreur sauvegarde diète');
  }
};
...
window.saveProgramme = async function(){
  var dayName = document.getElementById('prog-day-name')?.value?.trim();
  var muscles = document.getElementById('prog-day-muscles')?.value?.trim();
  var list = document.getElementById('prog-exos-list');
  var clientId = CLIENTS[activeClientIdx||0]?.id;

  if(!dayName){ toast2('Donne un nom à la séance'); return; }

  var exos = [];
  if(list){
    list.querySelectorAll('div[style]').forEach(function(row){
      var nom = row.querySelector('div > div:first-child')?.textContent;
      if(nom) exos.push({nom: nom});
    });
  }
  ...
  try {
    await sbUpsert('programmes_clients', {
      ...
      exercices: JSON.stringify(exos),
      actif: true
    }, 'client_id,jour_nom');
    toast2('Programme enregistré ✓');
  } catch(e) {
    toast2('Programme enregistré (local)');
  }
};
...
window.assignProg = async function(idx){
  ...
  var progCopy = JSON.parse(JSON.stringify(MES_PROGRAMMES[idx]));
  progCopy.id = 'prog-'+Date.now();
  progCopy.statut = 'prive';
  progCopy.clientId = CLIENTS[ci].id;
  progCopy.clientNom = CLIENTS[ci].name;
  progCopy.publishedAt = new Date().toISOString().split('T')[0];
  MES_PROGRAMMES.push(progCopy);
  saveProgToLS();
  document.getElementById('pc-inner').innerHTML = programmesHTML();
  // Sauvegarder en Supabase pour le client (remplace tout programme existant)
  try{
    await sbDelete('programmes_clients', 'client_id=eq.'+CLIENTS[ci].id);
    var rows = progCopy.jours.map(function(j,i){
      return {
    ...
    var res = await sbInsertBulk('programmes_clients', rows);
    if(!res.ok){
      console.error('assignProg erreur:', res.status, await res.text());
      toast2('Erreur assignation (voir console)');
      return;
    }
    toast2('Programme assigné à '+CLIENTS[ci].name+' ✓');
  }catch(e){
    console.error('assignProg:', e);
    toast2('Erreur assignation (voir console)');
  }
};
...
window.saveMacros = async function(){
  ...
  var gluc = document.getElementById('diet-gluc')?.value;
  var lip = document.getElementById('diet-lip')?.value;
  if(!kcal){ toast2('Renseigne au moins les kcal'); return; }
  try{
    // Supprimer l'ancienne diète puis réinsérer (évite les soucis de contrainte on_conflict)
    await sbDelete('dietes', 'client_id=eq.'+clientId);
    var res = await sbInsertBulk('dietes', [{
      client_id:clientId,
      kcal_jour:parseInt(kcal)||0,
    ...
    if(!res.ok){
      console.error('saveMacros erreur:', res.status, await res.text());
      toast2('Erreur sauvegarde (voir console)');
      return;
    }
    toast2('Macros enregistrées ✓ — visible côté client');
  }catch(e){
    console.error('saveMacros:', e);
    toast2('Erreur sauvegarde (voir console)');
  }
};
...
// Override addExoToProg pour sauvegarder dans PROG_JOURS_DATA
var _origAddExo = window.addExoToProg;
window.addExoToProg = function(){
  var sel = document.getElementById('prog-exo-select');
  var series = document.getElementById('prog-exo-series')?.value || '3';
  var reps = document.getElementById('prog-exo-reps')?.value || '10';
  var repos = document.getElementById('prog-exo-repos')?.value || '90s';
  var charge = document.getElementById('prog-exo-charge')?.value || '';
  var note = document.getElementById('prog-exo-note')?.value || '';
  if(!sel || !sel.value){ toast2('Choisis un exercice'); return; }
  if(!PROG_JOURS_DATA[PROG_JOUR_ACTIF]) PROG_JOURS_DATA[PROG_JOUR_ACTIF] = {nom:'Jour 1',muscles:'',exos:[]};
  PROG_JOURS_DATA[PROG_JOUR_ACTIF].exos.push({nom:sel.value,series:series,reps:reps,repos:repos,charge:charge,note:note});
  sel.value = '';
  ['prog-exo-series','prog-exo-reps','prog-exo-repos','prog-exo-charge','prog-exo-note'].forEach(function(id){
...
// Override saveProgramme pour inclure tous les jours
window.saveProgramme = async function(){
  var nameEl = document.getElementById('prog-day-name');
  var muscleEl = document.getElementById('prog-day-muscles');
  ...
  try{
    // 1. Supprimer l'ancien programme du client (approche fiable, evite les soucis de contrainte on_conflict)
    await sbDelete('programmes_clients', 'client_id=eq.'+clientId);
    // 2. Réinsérer tous les jours avec un ordre explicite
    var rows = PROG_JOURS_DATA.map(function(j,i){
      return {
        client_id: clientId,
        jour_nom: j.nom,
        muscles_cibles: j.muscles,
    ...
    if(!res.ok){
      ...
      console.error('saveProgramme erreur Supabase:', res.status, errTxt);
      toast2('Erreur sauvegarde (voir console)');
      return;
    }
    toast2('Programme enregistré ✓ — visible côté client');
  } catch(e){
    console.error('saveProgramme:', e);
    toast2('Erreur sauvegarde (voir console)');
  }
};
...
window.openProfile = function(id){
  ...
  if(avEl){ avEl.textContent = client.av || client.name[0]; avEl.style.background = client.bg||'var(--r)'; avEl.style.color = client.col||'#fff'; }

  // Mettre à jour pc-title
  var progNameEl = document.getElementById('prog-client-name');
  if(progNameEl) progNameEl.textContent = client.name;

  // Programme par défaut en attendant le chargement Supabase
  PROG_JOURS_DATA = [{nom:'Jour 1',muscles:'',exos:[]}];
  PROG_JOUR_ACTIF = 0;

  // Activer le premier onglet du panel
  var firstTab = panel ? panel.querySelector('.ptab') : null;
  ...
  // Charger le VRAI programme existant du client depuis Supabase
  sbFetch('programmes_clients?client_id=eq.'+client.id+'&select=*&order=ordre.asc,id.asc').then(function(rows){
    if(rows && rows.length){
      PROG_JOURS_DATA = rows.map(function(r){
    ...
    renderProgDayTabs();
    loadProgJour(0);
  }).catch(function(e){ console.error('Chargement programme client:', e); });

  // Charger les notes/victoires/vigilance/motivation existantes
  sbFetch('ecran_client?client_id=eq.'+client.id+'&select=*&limit=1').then(function(rows){
    var ec = rows && rows[0] ? rows[0] : {};
    var vic = []; try{ vic = JSON.parse(ec.victoires||'[]'); }catch(e){}
    var vig = []; try{ vig = JSON.parse(ec.points_attention||'[]'); }catch(e){}
    var vicEl = document.getElementById('victoires-input');
    var vigEl = document.getElementById('vigilance-input');
    var motivEl = document.getElementById('notes-motivation');
    var privEl = document.getElementById('notes-privees');
    if(vicEl) vicEl.value = vic.join('\n');
    if(vigEl) vigEl.value = vig.join('\n');
    if(motivEl) motivEl.value = ec.note_motivation || '';
    if(privEl) privEl.value = ec.notes_privees || '';
  }).catch(function(e){ console.error('Chargement notes client:', e); });

  // Charger la diète existante
  sbFetch('dietes?client_id=eq.'+client.id+'&actif=eq.true&select=*&limit=1').then(function(rows){
    var d = rows && rows[0] ? rows[0] : null;
    if(!d) return;
    var kcalEl=document.getElementById('diet-kcal'), protEl=document.getElementById('diet-prot'),
        glucEl=document.getElementById('diet-gluc'), lipEl=document.getElementById('diet-lip');
    if(kcalEl) kcalEl.value = d.kcal_jour || d.kcal || '';
    if(protEl) protEl.value = d.proteines_g || d.proteines || '';
    if(glucEl) glucEl.value = d.glucides_g || d.glucides || '';
    if(lipEl) lipEl.value = d.lipides_g || d.lipides || '';
  }).catch(function(e){ console.error('Chargement diete client:', e); });

  // Charger les messages
  loadMsgsCoach(client.id);
};
...
function pcNav(el, id){
  ...
  if(id === 'pc-agenda'){
    ...
    if(rdvDateEl) rdvDateEl.value = new Date().toISOString().split('T')[0];
    renderAgendaRDV(RDV_LIST);
    loadAgendaData();
  }
...
window.assignProgToClient = function(idx){
  var clients = CLIENTS.map(function(c){ return c.name; }).join('\n');
  var nom = prompt('Assigner le programme à quel client ?\n\nClients disponibles:\n' + clients);
  if(!nom) return;
  var client = CLIENTS.find(function(c){ return c.name.toLowerCase().includes(nom.toLowerCase()); });
  if(!client){ toast2('Client non trouvé'); return; }
  if(!CLIENTS_PROGRAMMES[client.id]) CLIENTS_PROGRAMMES[client.id] = [];
  CLIENTS_PROGRAMMES[client.id].push(MES_PROGRAMMES[idx].id);
  toast2('Programme assigné à ' + client.name + ' ✓');
};
...
function openProgEditor(){
  ...
  inner.innerHTML = '<div style="height:100%;display:flex;flex-direction:column;overflow:hidden">'
    ...
    +'<div style="flex:1"><div style="font-size:14px;font-weight:700">'+( PROG_EDIT.idx>=0?'Modifier':'Nouveau programme')+'</div></div>'
...
window.saveProgEdit = function(){
  ...
  if(PROG_EDIT.idx >= 0){
    MES_PROGRAMMES[PROG_EDIT.idx] = p;
  } else {
...
// ── AGENDA ──
window.changeVueAgenda = function(vue){
  window._agendaVueActive = vue;
  var inner = document.getElementById('pc-inner');
  if(!inner) return;
  inner.innerHTML = agendaView(vue);
  var d = document.getElementById('rdv-date');
...
function agendaView(vue){
  ...
  return '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px;flex-wrap:wrap;gap:8px">'
    ...
    +'<div class="frow"><div class="f1"><label class="lbl">Date</label><input type="date" class="inp" id="rdv-date" value="'+new Date().toISOString().split('T')[0]+'"></div>'
...
window.selectStatClient = async function(clientId){
  statsClientSelected = clientId || null;
  if(!clientId){document.getElementById('stats-client-detail').style.display='none';return;}
  document.getElementById('stats-client-detail').style.display='block';
  document.getElementById('stat-perte').textContent='...';
  document.getElementById('stat-prise').textContent='...';
  document.getElementById('stat-seances').textContent='...';
  var data = await loadClientStats(clientId);
  document.getElementById('stat-perte').textContent = data.perteMasse ? data.perteMasse+'kg' : '0kg';
  document.getElementById('stat-prise').textContent = data.priseMasse ? data.priseMasse+'kg' : '0kg';
  document.getElementById('stat-seances').textContent = data.seancesMois;
  var list = document.getElementById('stat-pesees-list');
  if(list){
    if(data.pesees.length > 0){
      var mois=['janv','fevr','mars','avr','mai','juin','juil','aout','sept','oct','nov','dec'];
      list.innerHTML = '<div style="font-size:9px;text-transform:uppercase;color:var(--txm);margin-bottom:6px">Historique pesées</div>'
        +data.pesees.slice(-5).reverse().map(function(p){
          var d = new Date(p.date_pesee);
          return '<div style="display:flex;justify-content:space-between;padding:5px 0;border-bottom:0.5px solid rgba(255,255,255,0.05)">'
            +'<span style="font-size:12px">'+d.getDate()+' '+mois[d.getMonth()]+' '+d.getFullYear()+'</span>'
            +'<span style="font-size:12px;font-weight:500;color:var(--gr)">'+p.poids_kg+' kg</span></div>';
        }).join('');
    } else {
      list.innerHTML = '<div style="font-size:12px;color:var(--txm);margin-top:8px">Aucune pesée enregistrée</div>';
    }
  }
};
...
// ── BASE EXOS ──
function dbHTML(){
  var groupsHtml = MUSCU.map(function(g, i){
    var exosHtml = g.e.map(function(e){
    ...
    return '<div style="margin-bottom:12px">'
      ...
      +'<span style="font-size:12px;font-weight:600">'+g.g+'</span>'
      +'<span style="font-size:10px;color:var(--txm)">'+g.e.length+' exos <i class="ti ti-chevron-down"></i></span>'
      +'</div>'
      +'<div style="display:'+(i===0?'block':'none')+';padding:0 4px">'+exosHtml+'</div>'
      +'</div>';
  }).join('');

  var myExosHtml = (window.MY_EXOS_PERSO||[]).length===0
    ? '<div style="font-size:11px;color:var(--txd);text-align:center;padding:16px 0">Aucun exercice — créez-en ci-dessous</div>'
    : (window.MY_EXOS_PERSO||[]).map(function(e,i){
      return '<div style="display:flex;align-items:center;justify-content:space-between;background:var(--c2);border-radius:8px;padding:8px 12px;margin-bottom:6px">'
        +'<div><div style="font-size:12px;font-weight:500">'+e.nom+'</div>'
        +'<div style="font-size:10px;color:var(--txm)">'+e.groupe+'</div></div>'
        +'<button onclick="deleteMyExo('+i+')" style="background:none;border:none;cursor:pointer;color:var(--txm)"><i class="ti ti-x" style="font-size:13px"></i></button>'
        +'</div>';
    }).join('');

  return '<div style="display:grid;grid-template-columns:1fr 320px;gap:16px;align-items:start">'
    +'<div>'
    +'<div style="font-size:11px;color:var(--txm);text-transform:uppercase;letter-spacing:0.7px;margin-bottom:12px">Bibliothèque · Cliquer sur un groupe pour dérouler</div>'
    +groupsHtml
    +'</div>'
    +'<div class="card" style="position:sticky;top:0">'
    +'<div class="sec" style="margin-bottom:12px">Créer un exercice</div>'
    +'<div style="margin-bottom:8px"><label class="lbl">Nom</label><input class="inp" id="nex-nm" placeholder="Ex : Curl barre EZ"></div>'
    +'<div style="margin-bottom:8px"><label class="lbl">Groupe musculaire</label>'
    +'<select class="inp" id="nex-grp">'+MUSCU.map(function(g){return '<option>'+g.g+'</option>';}).join('')+'</select></div>'
    +'<div style="margin-bottom:10px"><label class="lbl">Notes techniques</label>'
    +'<textarea class="inp" id="nex-note" rows="2" placeholder="Consignes clés..."></textarea></div>'
    +'<button class="btn-r full" onclick="creerExoPerso()"><i class="ti ti-plus" style="font-size:12px"></i>Ajouter l\'exercice</button>'
    +'<div style="margin-top:14px;border-top:0.5px solid rgba(255,255,255,0.06);padding-top:12px">'
    +'<div class="sec" style="margin-bottom:8px">Mes exercices créés</div>'
    +'<div id="my-exos-list">'+myExosHtml+'</div>'
    +'</div>'
    +'</div>'
    +'</div>';
}
...
window.confirmAddClient=function(){
  var email=document.getElementById('ac-email')?.value?.trim();
  var prenom=document.getElementById('ac-prenom')?.value?.trim();
  if(!prenom||!email){toast2('Remplis prénom et email au minimum');return;}
  closeAddClient();
  toast2('Compte créé — lien envoyé à '+email);
};
...
async function loadAgendaData(){
  ...
  sbFetch('rdv_coach?select=*,client:profiles!client_id(prenom,nom,email)&order=date_rdv.asc&limit=100')
    .then(function(rdvs){
...
function renderAgendaRDV(rdvs){
  ...
  var tomorrowStr = new Date(Date.now()+86400000).toISOString().split('T')[0];

  // Filtre : date_rdv >= aujourd'hui (comparaison string)
  var upcoming = (rdvs||[])
    .filter(function(r){ return r.date_rdv && r.date_rdv.substr(0,10) >= todayStr; })
    .sort(function(a,b){ return a.date_rdv.localeCompare(b.date_rdv); })
    .slice(0,15);
  ...
  cont.innerHTML = upcoming.map(function(rdv){
    var dateStr = rdv.date_rdv.substr(0,10);
    var heure   = rdv.date_rdv.substr(11,5) || '--:--';
    var isToday    = dateStr === todayStr;
    var isTomorrow = dateStr === tomorrowStr;
    // Calculer label sans timezone
    var parts = dateStr.split('-');
    var dLocal = new Date(parseInt(parts[0]), parseInt(parts[1])-1, parseInt(parts[2]));
    var label = isToday ? "Aujourd'hui" : isTomorrow ? 'Demain'
      : jours[dLocal.getDay()]+' '+dLocal.getDate()+' '+mois[dLocal.getMonth()];
    var nom = rdv.client ? ((rdv.client.prenom||'')+' '+(rdv.client.nom||'')).trim() : 'Client';
    if(!nom.trim()) nom = 'Client';
    var typeColor = rdv.type==='Présentiel'?'var(--or)':rdv.type==='Distanciel'?'var(--b)':'var(--gr)';
    return '<div data-rdv-id="'+rdv.id+'" style="display:flex;align-items:center;gap:10px;padding:10px 0;border-bottom:0.5px solid rgba(255,255,255,0.05);">'
      ...
      +'</div>'
      +'<button class="rdv-del-btn" style="background:none;border:none;cursor:pointer;color:var(--txm);padding:6px;flex-shrink:0;">'
      +'<i class="ti ti-trash" style="font-size:14px;"></i></button>'
      +'</div>';
  }).join('');

  // Attacher les listeners de suppression
  cont.querySelectorAll('.rdv-del-btn').forEach(function(btn){
    btn.addEventListener('click', function(){
      var row = btn.closest('[data-rdv-id]');
      var rdvId = row ? row.getAttribute('data-rdv-id') : null;
      if(!rdvId || !confirm('Supprimer ce RDV ?')) return;
      fetch(SB_URL+'/rest/v1/rdv_coach?id=eq.'+rdvId,{
        method:'DELETE',
        headers:{'apikey':SB_KEY,'Authorization':'Bearer '+SB_KEY,'Prefer':'return=minimal'}
      }).then(function(){
        RDV_LIST = RDV_LIST.filter(function(r){ return String(r.id)!==String(rdvId); });
        renderAgendaRDV(RDV_LIST);
        var inner = document.getElementById('pc-inner');
        if(inner && window._agendaVueActive) inner.innerHTML = agendaView(window._agendaVueActive);
        toast2('RDV supprimé ✓');
      }).catch(function(e){ toast2('Erreur: '+e.message); });
    });
  });
}
...
function pcNav(el,id){
  document.querySelectorAll('.ni').forEach(n=>n.classList.remove('on'));if(el)el.classList.add('on');
  var p=PC_PAGES[id];if(!p)return;
  document.getElementById('pc-title').textContent=p.t;
  document.getElementById('pc-sub').textContent=p.s;
  document.getElementById('pc-inner').innerHTML=p.h();
...
function pTab(el,id){document.querySelectorAll('.ptab').forEach(t=>t.classList.remove('on'));el.classList.add('on');document.querySelectorAll('.pscr').forEach(s=>s.classList.remove('on'));document.getElementById(id).classList.add('on');if(id==='pt-msgs'){var c=CLIENTS[activeClientIdx||0];if(c)loadMsgsCoach(c.id);}}
...
// ── MESSAGERIE ──
window.envoyerMsg = async function(){
  ...
  inp.value = '';
  try{
    await sbPost('messages', {client_id: clientId, sender: 'coach', contenu: contenu, created_at: new Date().toISOString()});
    await loadMsgsCoach(clientId);
  }catch(e){
    console.error('envoyerMsg:', e);
    toast2('Erreur envoi message');
  }
};

window.loadMsgsCoach = async function(clientId){
  var cont = document.getElementById('msgs-cont');
  if(!cont || !clientId) return;
  try{
    var rows = await sbFetch('messages?client_id=eq.'+clientId+'&select=*&order=created_at.asc&limit=100');
    if(!rows || !rows.length){
      cont.innerHTML = '<div style="font-size:12px;color:var(--txd);text-align:center;padding:20px 0">Aucun message</div>';
      return;
    }
    cont.innerHTML = rows.map(function(m){
      var isCoach = m.sender === 'coach';
      var d = new Date(m.created_at);
      var h = d.getHours().toString().padStart(2,'0')+':'+d.getMinutes().toString().padStart(2,'0');
      return '<div style="max-width:75%;align-self:'+(isCoach?'flex-end':'flex-start')+';padding:8px 11px;border-radius:'+(isCoach?'12px 12px 2px 12px':'12px 12px 12px 2px')+';font-size:12px;background:'+(isCoach?'var(--r)':'var(--c2)')+';color:'+(isCoach?'#fff':'var(--tx)')+';line-height:1.5">'
        + m.contenu
        + '<div style="font-size:9px;color:'+(isCoach?'rgba(255,255,255,0.6)':'var(--txd)')+';margin-top:2px">'+h+'</div></div>';
    }).join('');
    cont.scrollTop = cont.scrollHeight;
  }catch(e){
    console.error('loadMsgsCoach:', e);
  }
};
