...
window.assignProg = async function(progIdx){
  ...
  // 1. localStorage → affichage immédiat dans la fiche dashboard
  var key = 'ant_prog_client_' + cl.id;
  localStorage.setItem(key, JSON.stringify(jours));

  // 2. Supabase → visible dans l'app client (coaching.js)
  try{
    ...
    var rows = jours.map(function(j,i){
      return {
        client_id: cl.id,
  ...
  toast2('Programme "'+prog.nom+'" assigné à '+cl.name+' ✓');

  // Si la fiche est ouverte → recharger
  if(activeClientIdx === ci){
    PROG_JOURS_DATA = JSON.parse(JSON.stringify(jours));
    PROG_JOUR_ACTIF = 0;
    renderProgDayTabs();
    loadProgJour(0);
  }
