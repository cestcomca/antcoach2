...
async function sbFetch(path){
  var _sess = getSBSession();
  var _tok = (_sess && _sess.access_token) ? _sess.access_token : SB_KEY;
  const r = await fetch(SB_URL+'/rest/v1/'+path, {
    headers: {'apikey': SB_KEY, 'Authorization': 'Bearer '+_tok, 'Content-Type': 'application/json'}
  });
...
window.enregistrerRdv = async function(){
  try{
    ...
    // Rafraîchir la liste ET la vue agenda
    sbFetch('rdv_coach?select=*,client:profiles!client_id(prenom,nom,email)&order=date_rdv.asc&limit=50')
      .then(function(rdvs){
        if(!Array.isArray(rdvs)) return;
        RDV_LIST = rdvs;
        renderAgendaRDV(rdvs);
        // Mettre à jour la vue jour/semaine/mois si affichée
        var inner = document.getElementById('pc-inner');
        if(inner && inner.querySelector('.rdv-cols, .cal-grid, [class*="rdv"]')){
          inner.innerHTML = agendaView(window._agendaVueActive || 'semaine');
        }
      }).catch(function(){});
...
window.assignProg = async function(progIdx){
  progIdx = parseInt(progIdx);
  ...
  var cl = CLIENTS[ci];
  var prog = MES_PROGRAMMES[progIdx];
  if(!prog){ toast2('Programme introuvable'); return; }
  var jours = prog.jours || [];
  if(!jours.length){ toast2('Programme sans jours'); return; }

  // Vérifier si le client a déjà un programme
  var key = 'ant_prog_client_' + cl.id;
  var existingJours = [];
  try{
    var saved = localStorage.getItem(key);
    if(saved) existingJours = JSON.parse(saved);
  }catch(e){}

  var finalJours = [];
  if(existingJours.length > 0){
    var action = confirm(
      cl.name + ' a déjà ' + existingJours.length + ' jour(s) dans son programme.\n\n' +
      'OK = Ajouter "' + prog.nom + '" à son programme existant\n' +
      'Annuler = Remplacer complètement son programme'
    );
    if(action){
      // Ajouter
      finalJours = existingJours.concat(JSON.parse(JSON.stringify(jours)));
    } else {
      // Remplacer
      finalJours = JSON.parse(JSON.stringify(jours));
    }
  } else {
    finalJours = JSON.parse(JSON.stringify(jours));
  }
  ...
  localStorage.setItem(key, JSON.stringify(finalJours));

  // Sauvegarder Supabase pour l'app client
  try{
    await fetch(SB_URL+'/rest/v1/programmes_clients?client_id=eq.'+cl.id,{
      method:'DELETE',
      headers:{'apikey':SB_KEY,'Authorization':'Bearer '+SB_KEY,'Prefer':'return=minimal'}
    });
    var rows = finalJours.map(function(j,i){
      return {
        client_id: cl.id,
    ...
    await fetch(SB_URL+'/rest/v1/programmes_clients',{
      method:'POST',
      headers:{'apikey':SB_KEY,'Authorization':'Bearer '+SB_KEY,'Content-Type':'application/json','Prefer':'return=minimal'},
      body: JSON.stringify(rows)
    });
  }catch(e){ console.log('assignProg Supabase:', e.message); }

  toast2('Programme assigné à '+cl.name+' ✓ ('+finalJours.length+' jour(s))');

  // Si la fiche est ouverte → recharger
  if(activeClientIdx === ci){
    PROG_JOURS_DATA = JSON.parse(JSON.stringify(finalJours));
    PROG_JOUR_ACTIF = 0;
    renderProgDayTabs();
    loadProgJour(0);
  }
...
function loadProgJour(i){
  ...
  if(listEl){
    listEl.innerHTML = (j.exos||[]).map(function(e,ei){
      var note = e.note_client || '';
      return '<div style="background:var(--c2);border-radius:10px;padding:10px 12px;margin-bottom:8px;">'
        +'<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:6px;">'
        +'<div>'
        +'<div style="font-size:13px;font-weight:500;">'+e.nom+'</div>'
        +'<div style="font-size:10px;color:var(--txm);">'+e.series+' × '+e.reps+' · repos '+e.repos+(e.charge?' · '+e.charge+'kg':'')+'</div>'
        +'</div>'
        +'<button data-ei="'+ei+'" class="del-exo-btn" style="background:none;border:none;cursor:pointer;color:var(--txm);padding:4px;"><i class="ti ti-x" style="font-size:13px;"></i></button>'
        +'</div>'
        +'<input type="text" data-ei="'+ei+'" class="note-client-inp inp" placeholder="Note technique pour ce client..." value="'+note.replace(/"/g,'&quot;')+'" style="font-size:11px;padding:6px 10px;">'
        +'</div>';
    }).join('') || '<div style="font-size:11px;color:var(--txd);text-align:center;padding:12px 0;">Aucun exercice — ajoutez-en ci-dessous</div>';

    // Listeners supprimer
    listEl.querySelectorAll('.del-exo-btn').forEach(function(btn){
      btn.addEventListener('click', function(){
    ...
    // Listeners note technique client
    listEl.querySelectorAll('.note-client-inp').forEach(function(inp){
      inp.addEventListener('change', function(){
        var ei = parseInt(inp.getAttribute('data-ei'));
        if(PROG_JOURS_DATA[PROG_JOUR_ACTIF] && PROG_JOURS_DATA[PROG_JOUR_ACTIF].exos[ei]){
          PROG_JOURS_DATA[PROG_JOUR_ACTIF].exos[ei].note_client = inp.value;
          saveClientProg();
        }
      });
    });
  ...
  // Listener : nom de séance → mise à jour en temps réel
  var nameEl2 = document.getElementById('prog-day-name');
  if(nameEl2){
    nameEl2.oninput = function(){
      if(PROG_JOURS_DATA[PROG_JOUR_ACTIF]){
        PROG_JOURS_DATA[PROG_JOUR_ACTIF].nom = nameEl2.value;
        // Mettre à jour le bouton de l'onglet actif
        var activeTab = document.querySelector('.dtab.on');
        if(activeTab) activeTab.textContent = nameEl2.value || ('Jour '+(PROG_JOUR_ACTIF+1));
        saveClientProg();
      }
    };
  }
  // Listener : muscles → mise à jour en temps réel
  var muscleEl2 = document.getElementById('prog-day-muscles');
  if(muscleEl2){
    muscleEl2.oninput = function(){
      if(PROG_JOURS_DATA[PROG_JOUR_ACTIF]){
        PROG_JOURS_DATA[PROG_JOUR_ACTIF].muscles = muscleEl2.value;
        saveClientProg();
      }
    };
  }
...
// Sauvegarder le programme du client actif
function saveClientProg(){
  var cl = CLIENTS[activeClientIdx];
  if(!cl) return;
  var key = 'ant_prog_client_' + cl.id;
  try{ localStorage.setItem(key, JSON.stringify(PROG_JOURS_DATA)); }catch(e){}
}
...
window.openProfile = function(id){
  ...
  // Charger le programme depuis localStorage
  var key = 'ant_prog_client_' + id;
  try{
    var saved = localStorage.getItem(key);
    if(saved){
      PROG_JOURS_DATA = JSON.parse(saved);
    } else {
  ...
  // Charger les notes (async séparé, ne bloque pas l'affichage)
  setTimeout(async function(){
    try{
      var notes = await sbFetch('ecran_client?client_id=eq.'+id+'&select=*&limit=1');
      if(notes && notes[0]){
        var nm = document.getElementById('notes-motivation');
        var vi = document.getElementById('victoires-input');
        var vg = document.getElementById('vigilance-input');
        var np = document.getElementById('notes-privees');
        if(nm) nm.value = notes[0].note_motivation||notes[0].note_visible||'';
        if(vi) vi.value = (JSON.parse(notes[0].victoires||'[]')).join('\n');
        if(vg) vg.value = (JSON.parse(notes[0].points_attention||'[]')).join('\n');
        if(np) np.value = notes[0].notes_privees||'';
      }
    }catch(e){}
  }, 100);
};
...
window.changeVueAgenda=function(vue){
  ...
  var i=document.getElementById("pc-inner");
  if(i){
    i.innerHTML=agendaView(vue);
    setTimeout(function(){
...
function agendaView(vue){
  ...
  if(v === 'jour'){
    ...
    var rdvHtml = rdvDuJour.length ? rdvDuJour.map(function(rdv){
      var h = rdv.date_rdv ? rdv.date_rdv.substr(11,5).replace(':','h') : '?h??';
...
async function loadAgendaData(){
  try{
    var rdvs = await sbFetch('rdv_coach?select=*,client:profiles!client_id(prenom,nom,email)&order=date_rdv.asc&limit=100');
    ...
    console.log('loadAgendaData erreur:', e.message);
    // Afficher quand même avec ce qu'on a
    renderAgendaRDV(RDV_LIST);
  }
}
...
function renderAgendaRDV(rdvs){
  ...
  // Filtrer les RDV à venir (date >= aujourd'hui) en comparant les strings
  var upcoming = (rdvs||[])
  ...
  cont.innerHTML = upcoming.map(function(rdv){
    ...
    var heure   = rdv.date_rdv.substr(11,5).replace(':','h');
    var isToday    = dateStr === todayStr;
    var tomorrowStr = new Date(Date.now()+86400000).toISOString().split('T')[0];
    var isTomorrow = dateStr === tomorrowStr;
    // Calculer le jour depuis la string (sans timezone)
    var parts = dateStr.split('-');
    var dLocal = new Date(parseInt(parts[0]), parseInt(parts[1])-1, parseInt(parts[2]));
    var label = isToday ? "Aujourd\'hui" : isTomorrow ? 'Demain' : jours[dLocal.getDay()]+' '+dLocal.getDate()+' '+mois[dLocal.getMonth()];
    ...
    return '<div style="display:flex;align-items:center;gap:10px;padding:10px 0;border-bottom:0.5px solid rgba(255,255,255,0.05);">'
      ...
      +'</div>'
      +'<button data-rdv-id="'+rdv.id+'" class="del-rdv-btn" title="Supprimer" style="background:none;border:none;cursor:pointer;color:var(--txm);padding:6px;flex-shrink:0;">'
      +'<i class="ti ti-trash" style="font-size:14px;"></i></button>'
      +'</div>';
  }).join('');

  // Attacher les listeners supprimer
  cont.querySelectorAll('.del-rdv-btn').forEach(function(btn){
    btn.addEventListener('click', function(){
      var rdvId = btn.getAttribute('data-rdv-id');
      ...
      fetch(SB_URL+'/rest/v1/rdv_coach?id=eq.'+rdvId,{
        ...
        renderAgendaRDV(RDV_LIST);
        // Rafraîchir la vue agenda si ouverte
        var inner = document.getElementById('pc-inner');
        if(inner && window._agendaVueActive){
          inner.innerHTML = agendaView(window._agendaVueActive);
        }
        toast2('RDV supprimé ✓');
...
function renderRDVAgenda(){
  ...
  RDV_LIST.forEach(function(rdv){
    ...
    var heure = rdv.date_rdv ? rdv.date_rdv.substr(11,5) : '?:??';
...
window.deleteExo=function(gi,ei){
  if(!confirm('Supprimer cet exercice ?')) return;
  MUSCU[gi].e.splice(ei,1);
  saveMyExosToLS();
  document.getElementById('pc-inner').innerHTML=dbHTML();
  toast2('Exercice supprimé ✓');
};
...
window.editExo=function(gi,ei){
  ...
  modal.innerHTML='<div style="background:var(--c1);border-radius:16px;padding:20px;width:100%;max-width:340px;">'
    +'<div style="font-size:14px;font-weight:600;margin-bottom:14px;color:var(--tx);">'+n.trim()+'</div>'
    +(photo?'<img src="'+photo+'" style="width:100%;height:100px;object-fit:cover;border-radius:8px;margin-bottom:10px;">':'')
    +'<div style="font-size:11px;color:var(--txm);margin-bottom:6px;">Nouvelle photo (optionnel) :</div>'
    +'<input type="file" id="edit-exo-photo-input" accept="image/*" style="width:100%;font-size:12px;color:var(--tx);margin-bottom:14px;">'
    +'<div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;">'
    +'<button onclick="saveEditExo(false)" style="padding:10px;background:var(--c2);border:none;border-radius:10px;color:var(--tx);font-size:12px;font-weight:600;cursor:pointer;font-family:inherit;">Sans photo</button>'
    +'<button onclick="saveEditExo(true)" style="padding:10px;background:var(--r);border:none;border-radius:10px;color:#fff;font-size:12px;font-weight:600;cursor:pointer;font-family:inherit;">Avec photo</button>'
    +'</div>'
    +'<button onclick="var m=document.getElementById(\'modal-edit-exo\');if(m)m.remove();" style="width:100%;margin-top:8px;padding:7px;background:none;border:none;color:var(--txm);font-size:11px;cursor:pointer;font-family:inherit;">Annuler</button>'
    +'</div>';
...
window.saveEditExo=function(useNewPhoto){
  ...
  if(useNewPhoto){
    ...
    if(!inp||!inp.files||!inp.files[0]){toast2('Sélectionne une photo');return;}
    var reader=new FileReader();
    reader.onload=function(ev){
      MUSCU[ex.gi].e[ex.ei]={nom:ex.nom,note:ex.note,photo:ev.target.result};
      saveMyExosToLS();
      var m=document.getElementById('modal-edit-exo');if(m)m.remove();
      document.getElementById('pc-inner').innerHTML=dbHTML();
...
window.analyserClientData = async function(){
  var cl = CLIENTS[activeClientIdx];
  if(!cl){ toast2('Ouvre une fiche client'); return; }
  toast2('Analyse en cours...');
  var sugg_vic = [], sugg_vig = [];
  try{
    var mois_debut = new Date(new Date().getFullYear(), new Date().getMonth(), 1).toISOString().split('T')[0];
    var mois_fr = ['janvier','février','mars','avril','mai','juin','juillet','août','septembre','octobre','novembre','décembre'];
    var nom_mois = mois_fr[new Date().getMonth()];

    // Séances du mois
    var xp = [];
    try{ xp = await sbFetch('xp_log?client_id=eq.'+cl.id+'&created_at=gte.'+mois_debut+'&select=source'); }catch(e){}
    var nb_seances = (xp||[]).filter(function(x){return x.source==='seance_exo';}).length;
    var nb_defis   = (xp||[]).filter(function(x){return x.source==='defi';}).length;

    // Toujours une victoire sur les séances
    if(nb_seances >= 12)     sugg_vic.push(nb_seances+' entraînements en '+nom_mois+' — exceptionnel ! 🏆');
    else if(nb_seances >= 8) sugg_vic.push(nb_seances+' entraînements en '+nom_mois+' — super régularité ! 🔥');
    else if(nb_seances >= 4) sugg_vic.push(nb_seances+' séances en '+nom_mois+', continue comme ça ! 💪');
    else if(nb_seances >= 2) sugg_vic.push(nb_seances+' séances en '+nom_mois+' — bon début, on continue ! 👊');
    else if(nb_seances === 1) sugg_vic.push('1 séance en '+nom_mois+' — chaque pas compte ! 🚀');
    else sugg_vic.push('Nouveau mois, nouvelle chance de briller ! 💥');

    if(nb_seances < 4) sugg_vig.push('Seulement '+nb_seances+' séance(s) en '+nom_mois+' — on peut faire mieux !');
    if(nb_defis >= 20) sugg_vic.push(nb_defis+' défis validés ce mois 🎯');
    else if(nb_defis >= 10) sugg_vic.push(nb_defis+' défis validés — belle assiduité !');
    else if(nb_defis < 5)   sugg_vig.push('Peu de defis valides ce mois ('+nb_defis+') - essaie d\'en faire plus !');

    // Suivi eau/sommeil/pas
    var suivi = [];
    try{ suivi = await sbFetch('suivi_quotidien?client_id=eq.'+cl.id+'&date_suivi=gte.'+mois_debut+'&select=eau_verres,sommeil_h,pas'); }catch(e){}
    if(suivi && suivi.length > 0){
      var n = suivi.length;
      var eau = suivi.reduce(function(s,r){return s+(r.eau_verres||0);},0)/n;
      var som = suivi.reduce(function(s,r){return s+(parseFloat(r.sommeil_h)||0);},0)/n;
      var pas = suivi.reduce(function(s,r){return s+(r.pas||0);},0)/n;

      if(eau >= 7)      sugg_vic.push('Hydratation parfaite : '+eau.toFixed(1)+' verres/jour 💧');
      else if(eau >= 5) sugg_vic.push('Bonne hydratation : '+eau.toFixed(1)+' verres/jour en moyenne');
      else if(eau >= 3) sugg_vic.push('Hydratation à améliorer : '+eau.toFixed(1)+' verres/jour — vise 6+');
      else              sugg_vig.push('Hydratation insuffisante : '+eau.toFixed(1)+' verres/jour');

      if(som >= 8)        sugg_vic.push('Excellent sommeil : '+som.toFixed(1)+'h/nuit 😴');
      else if(som >= 7)   sugg_vic.push('Bon sommeil : '+som.toFixed(1)+'h/nuit en moyenne');
      else if(som >= 6)   sugg_vig.push('Sommeil un peu court : '+som.toFixed(1)+'h/nuit — vise 7-8h');
      else                sugg_vig.push('Manque de sommeil : '+som.toFixed(1)+'h/nuit — priorité récupération !');

      if(pas >= 10000)    sugg_vic.push('Très actif : '+Math.round(pas).toLocaleString('fr')+' pas/jour 🚶');
      else if(pas >= 7000) sugg_vic.push('Bonne activité quotidienne : '+Math.round(pas).toLocaleString('fr')+' pas/jour');
      else if(pas >= 4000) sugg_vic.push('Activité en cours : '+Math.round(pas).toLocaleString('fr')+' pas/jour — vise 8000 !');
      else if(pas > 0)     sugg_vig.push('Peu de marche : '+Math.round(pas).toLocaleString('fr')+' pas/jour — bouge plus !');
    }

    // Évolution poids
    var pesees = [];
    try{ pesees = await sbFetch('pesees?client_id=eq.'+cl.id+'&order=date_pesee.asc&select=poids_kg&limit=20'); }catch(e){}
    if(pesees && pesees.length >= 2){
      var diff = (pesees[pesees.length-1].poids_kg - pesees[0].poids_kg).toFixed(1);
      if(diff <= -3)      sugg_vic.push('Perte de '+Math.abs(diff)+'kg depuis le début — objectif en vue ! 📉');
      else if(diff <= -1) sugg_vic.push('En bonne progression : -'+Math.abs(diff)+'kg depuis le début');
      else if(diff >= 2)  sugg_vic.push('Prise de masse : +'+diff+'kg — la force arrive ! 💪');
    }

  }catch(e){
    console.log('analyserClientData:', e);
  }

  // Suggestions manuelles toujours présentes
  sugg_vic.push('À compléter : Record sur [exercice] — [charge]kg ✓');
  sugg_vic.push('À compléter : Bonne séance de [muscle] cette semaine !');
  sugg_vig.push('À compléter : Baisse de charge sur [exercice] — à surveiller');
  sugg_vig.push('À compléter : Écart alimentaire cette semaine — on reprend !');

  // Afficher les suggestions
  var vl = document.getElementById('sugg-vic-list');
  var gl = document.getElementById('sugg-vig-list');
  if(vl){
    vl.innerHTML = '';
    sugg_vic.forEach(function(s){
      var btn = document.createElement('button');
      btn.textContent = s;
      btn.style.cssText = 'padding:5px 10px;background:rgba(76,175,122,0.15);border:1px solid rgba(76,175,122,0.4);border-radius:20px;color:#4CAF7A;font-size:10px;cursor:pointer;font-family:inherit;margin:2px;text-align:left;';
      btn.addEventListener('click', function(){ ajouterSuggestion('victoires-input', s); });
      vl.appendChild(btn);
    });
  }
  if(gl){
    gl.innerHTML = '';
    sugg_vig.forEach(function(s){
      var btn = document.createElement('button');
      btn.textContent = s;
      btn.style.cssText = 'padding:5px 10px;background:rgba(232,149,58,0.15);border:1px solid rgba(232,149,58,0.4);border-radius:20px;color:#E8953A;font-size:10px;cursor:pointer;font-family:inherit;margin:2px;text-align:left;';
      btn.addEventListener('click', function(){ ajouterSuggestion('vigilance-input', s); });
      gl.appendChild(btn);
    });
  }
