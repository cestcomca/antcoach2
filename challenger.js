...
// ══ TOAST ══
function toast(msg){var t=document.getElementById('toast-el');var _tt=document.getElementById('toast-txt');if(_tt)_tt.textContent=msg;t.classList.add('show');setTimeout(()=>t.classList.remove('show'),2500);}
...
function updateAll(){
  ...
  lastLvl=info.lvl;
  var _ln=document.getElementById('lh-num');if(_ln)_ln.textContent=info.lvl;
  document.getElementById('xp-fill').style.width=pct+'%';
  var _xc=document.getElementById('xp-cur');if(_xc)_xc.textContent=info.xpIn+' XP';
  var _xn=document.getElementById('xp-need');if(_xn)_xn.textContent='niv.'+(info.lvl+1)+' : '+info.need+' XP';
  var _xt=document.getElementById('xp-today');if(_xt)_xt.textContent=todayXP;
  var _hx=document.getElementById('home-xp');if(_hx)_hx.textContent=totalXP;
  var _ml=document.getElementById('mini-lvl');if(_ml)_ml.textContent=info.lvl;
  var _mc=document.getElementById('mini-xp-cur');if(_mc)_mc.textContent=info.xpIn+' XP';
  var _mn=document.getElementById('mini-xp-need');if(_mn)_mn.textContent='niv.'+(info.lvl+1)+' : '+info.need+' XP';
  document.getElementById('mini-bar').style.width=pct+'%';
  var _xco=document.getElementById('xp-count');if(_xco)_xco.textContent=todayXP;
  var _hd=document.getElementById('home-defis');if(_hd)_hd.innerHTML=done+'<span style="font-size:12px;color:var(--txm)">/5</span>';
  var _dd=document.getElementById('defis-done-count');if(_dd)_dd.textContent=done;
  var _bs=document.getElementById('b-sleep-d');if(_bs)_bs.textContent=sleep+'h';
  var _bst=document.getElementById('b-steps-d');if(_bst)_bst.textContent=steps.toLocaleString('fr');
  var _bw=document.getElementById('b-water-d');if(_bw)_bw.textContent=water+' verres';
...
// ══ DÉCONNEXION ══
function logout(){closeModal('modal-profil');window.location.href='hub.html';}
...
function editJour(i){
  ...
  var list=document.getElementById('builder-exos-list');if(list)list.innerHTML=(j.exos||[]).map(function(e,idx){
    return '<div style="display:flexalign-items:center;justify-content:space-between;padding:7px 0;border-bottom:0.5px solid rgba(255,255,255,0.05)">'
