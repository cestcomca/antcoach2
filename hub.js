...
async function init() {
  ...
  if(params.get('checkout') === 'success'){
    ...
    msg.innerHTML = '<div style="font-family:Bebas Neue,sans-serif;font-size:36px;color:#FFD700;letter-spacing:3px">ABONNEMENT ACTIVÉ !</div>'
      ...
      + '<a href="ant_coach_app_challenger.html" style="background:#8B0000;color:#fff;padding:14px 28px;border-radius:10px;font-size:14px;font-weight:600;text-decoration:none;margin-top:8px">Accéder à mon espace →</a>'
...
async function redirectByRole(user) {
  ...
  const map = {
    ...
    coaching_presentiel: 'coaching/coaching.html',
    coaching_distanciel: 'coaching/coaching.html',
    challenger:          'challenger/challenger.html',
    challenger_elite:    'ant_coach_app_challenger_elite.html',
  }
  const { data: { session } } = await sb.auth.getSession()
  var token = session ? session.access_token : ''
  var uid = session ? session.user.id : ''
  var dest = map[role] || 'challenger/challenger.html'
